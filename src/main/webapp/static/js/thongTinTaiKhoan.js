...
async function getUser(callback){
    let api = 'https://localhost:8443/api/user?id=' + sessionStorage.getItem('idUser');
    await fetch(api)
...
async function updateUser(formData){
    let api = "https://localhost:8443/api/user";
    await fetch(api, {
        method: 'PUT',
        body: formData
    })
...
async function checkPassWord(){
    ...
    let api = 'https://localhost:8443/api/user?id=' + idUser + "&mat-khau=" + password;
    let _data;
...
async function updatePassWord(idUser, password){
    let api = 'https://localhost:8443/api/user/doi-mat-khau';
    let formData = new FormData();
